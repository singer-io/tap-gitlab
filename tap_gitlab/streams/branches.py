...
class Branches(ChildBaseStream):
    ...
    parent = "projects"
    replication_keys = ["last_commited_date"]
    path = "projects/{}/repository/branches"
    data_key = None
    bookmark_value = None

    def modify_object(self, record, parent_record = None):
        """Adding project_id and last_commited_date to the record"""
        if isinstance(record, dict):
            ...
            if commit and isinstance(commit, dict):
                ...
                if committed_date:
                    record["last_commited_date"] = committed_date
                else:
                    record["last_commited_date"] = self.client.config.get("start_date")
            else:
                record["last_commited_date"] = self.client.config.get("start_date")
